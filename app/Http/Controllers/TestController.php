...
class TestController extends Controller
{
    ...
    protected function get_my_tests($type, $test_id = 0)
    {
        ...
        //     }
        // }
        $tests = Test::where('for', 'like', '%'.Auth::id().'%')->get()->toArray();
        foreach ($tests as $x){
            array_push($filtered_tests, $x['id']);
        }

        // if ($type == 'bool') {
        //     foreach ($filtered_tests as $filtered_test) {
        //         $for_me = in_array($test_id, $filtered_test);
        ...
        // }
        if($type == 'bool'){
            $for_me = in_array($test_id, $filtered_tests);
    ...
    public function upload_screen(Test $test, Request $request){
        try{
            if($request->has('image_data') && ($request->has('proctor_data'))){
                $image = $request->input('image_data');
                $imageName = time(). '.'.'png';
                \File::put(public_path('images').'/'.$imageName, base64_decode($image));
                $filepath = asset('/images'.'/'.$imageName);

                $proctor_data = json_encode($request->input('proctor_data'));
                $proctorResult = ObjectDetection::create([
                    'user_id' => Auth::id(),
                    'test_id' => $test->id,
