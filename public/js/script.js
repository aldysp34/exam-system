...
async function captureScreen(){
    ...
    setInterval(() => {
        ...
        let screenData = canvas.toDataURL('image/png').split(',')[1]
        const paddingNeeded = 4 - (screenData.length % 4);
        if (paddingNeeded > 0) {
            screenData += '='.repeat(paddingNeeded);
        }
        $.ajax({
            type: 'POST',
            ...
            success: function(response){
                console.log('success get proctor data')
                console.log(response.payload)
                $.ajax({
                    type: 'POST',
                    url: "/cbt/upload_screen/" + $("#test_id").val(),
                    headers: {
                        "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
                    },
                    data: {
                        image_data: screenData,
                        proctor_data: response.payload
                    },
                    success: function(response){
            ...
            error: function(response){
                console.log("error get proctor data")
                console.log(response)
            }
        });
    }, 30000);
